...
function checkChart(secs) {
    ...
    { // Check required sections
        const requiredSectionError = checkRequiredSections(secs, [Meta.ESection.SONG, Meta.ESection.SYNC_TRACK], trackNames);
    ...
    { // Check Events section
        ...
        if (eventsSection) {
            const eventsSectionTypeError = checkEventTypes(Meta.ESection.EVENTS, Meta.EventTypes, eventsSection.content);
...
function checkRequiredSections(secs, required, oneOf) {
    ...
    if (!hasOneSectionOf) {
        return {
            reason: Error_1.getErrorString(Error_1.EError.MISSING_ONE_OF, {
                sections: oneOf
            })
        };
    }
...
function checkLyricsPhrases(content) {
    ...
    if (errorItem) {
        ...
        return ({
            reason: Error_1.getErrorString(Error_1.EError.WRONG_LYRICS, {
                item: errorItem,
                found: eventSubtype
            })
        });
...
function checkRepeatedNoteEvent(section, ticks) {
    ...
    if (tickRepeatedEventError) {
        ...
        return ({
            reason: Error_1.getErrorString(Error_1.EError.DUPLICATE_NOTE_EVENT, {
                section,
...
function checkNoteFlags(section, ticks) {
    const flagError = Object.keys(ticks).find(tick => {
        ...
        const tickIsFlagged = itemsOnSameTick.some(item => {
            ...
            return (value === Meta.EGuitarNoteEventType.FORCED
    ...
    if (flagError) {
        ...
        return ({
            reason: Error_1.getErrorString(Error_1.EError.WRONG_NOTE_FLAG, {
                section,
